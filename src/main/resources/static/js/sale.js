...
async function updateDashboard() {
  ...
  try {
    state.loading = true;
    setNotice("선택한 지역의 국토교통부 매매 실거래가 API 데이터를 불러오는 중입니다.");
    ...
    renderRanking();
    renderDeals();
    setNotice("국토교통부 매매 실거래가 API와 연결된 데이터입니다.");
  } catch (error) {
    console.error(error);
...
async function getSummary(regionCode) {
  ...
  if (!res.ok || !data.ok) {
    throw new Error(data.message || "매매 데이터를 불러오지 못했습니다.");
  }
...
function renderDashboard(base, compare) {
  ...
  const baseRate = Number(base.saleChangeRate || 0);
  const compareRate = Number(compare.saleChangeRate || 0);
  const baseTradeCount = Number(base.tradeCount ?? base.sampleTradeList?.length ?? 0);
  const baseVolumeRate = Number(base.volumeChangeRate ?? 0);

  $("#averagePrice").textContent = money(basePrice);
  $("#averageChange").innerHTML = rateHtml(baseRate, "전월 대비");
  $("#pricePerPyeong").textContent = `${estimatePyeongPrice(basePrice).toLocaleString()}만원`;
  $("#pyeongDiff").textContent = `${compareName} 대비 ${Math.abs(estimatePyeongPrice(basePrice) - estimatePyeongPrice(comparePrice)).toLocaleString()}만원 ${basePrice >= comparePrice ? "높음" : "낮음"}`;
  $("#tradeVolume").textContent = `${baseTradeCount.toLocaleString()}건`;
  $("#volumeChange").innerHTML = rateHtml(baseVolumeRate, "전월 대비");
  ...
  $("#comparisonInsight").textContent = `${baseName}의 평균 매매가는 ${compareName}보다 ${Math.abs(gap / 10000).toFixed(1)}억원 ${gap >= 0 ? "높고" : "낮고"}, 최근 상승률은 ${Math.abs(baseRate - compareRate).toFixed(1)}%p 차이입니다.`;
...
function renderChart(base, compare, period) {
  ...
  const height = 300;
  const pad = 42;
  const baseSeries = makeSeries(base.saleAvg, base.saleChangeRate, period);
  const compareSeries = makeSeries(compare.saleAvg, compare.saleChangeRate, period);
  const all = [...baseSeries, ...compareSeries].filter(Boolean);
  const min = Math.max(0, Math.min(...all) - 8000);
  const max = Math.max(...all, 1) + 8000;
...
function renderRanking() {
  const rows = [...state.summaries.values()]
    .filter(item => item.saleAvg)
    .sort((a, b) => Number(b.saleAvg) - Number(a.saleAvg));
  const max = Math.max(...rows.map(row => Number(row.saleAvg || 0)), 1);

  $("#rankingList").innerHTML = rows.map((row, index) => `
    <div class="ranking-row">
      <b>${index + 1}</b>
      <span>${escapeHtml(row.regionName)}</span>
      <div><i style="width:${Number(row.saleAvg || 0) / max * 100}%"></i></div>
      <strong>${money(row.saleAvg)}</strong>
      <em>${formatRate(row.saleChangeRate)}</em>
    </div>
  `).join("");
}
...
function renderDeals() {
  ...
  const renderedGroups = groups.map(([label, summary]) => {
    const deals = (summary.sampleTradeList || [])
  ...
  $("#emptyDeals").hidden = groups.some(([, summary]) => (summary.sampleTradeList || [])
    .filter(item => areaMatches(item.area))
          ...
          <tbody>
            ${deals.length ? deals.map(item => `
              <tr>
                ...
                <td class="${Number(item.amount || 0) >= Number(summary.saleAvg || 0) ? "up" : "down"}">${Number(item.amount || 0) >= Number(summary.saleAvg || 0) ? "▲" : "▼"} 지역 평균 대비</td>
              </tr>
...
function estimatePyeongPrice(price) {
  return Math.round(Number(price || 0) / 25.7);
}
