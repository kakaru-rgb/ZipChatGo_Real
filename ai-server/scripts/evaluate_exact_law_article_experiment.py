...
EVALUATION_MODE = "production_agent_passthrough_law_name_filter_exact_lookup_experiment"
EXPERIMENT_FIELDS = (
    "적용된LawNames",
    "탐지LawArticlePairs",
    "ExactLookupAttempted",
    "ExactLookupHit",
    "ExactLookupResult",
    "SemanticFallback여부",
    "ExactLookupTrace",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run evaluation-only A+B exact law/article experiment.")
    parser.add_argument("--input-csv", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--question-nos", default="6,9,21,27,37")
    ...
    run_id = f"law-exact-{evaluation_timestamp().replace(':', '').replace('+', '_')}-{uuid.uuid4().hex[:8]}"
    ...
    for index, item in enumerate(selected, start=1):
        ...
        runtime, experiment, capture = _build_runtime(
            api_key, vector_store_id, args.model, question.question
        )
        ...
        row.update(_experiment_fields(experiment))
    ...
    print(f"Saved {len(rows)} A+B experimental rows: {output_path}")
    return 0
...
def _build_runtime(
    ...
    model: str,
    question_text: str,
) -> tuple[PassthroughRuntime, ExactLawArticleExperimentHandler, PreValidationCapture]:
    ...
    runtime = PassthroughRuntime(
        ...
        handlers={
            ...
            "search_real_estate_law": experiment,
        },
    )
    return runtime, experiment, capture
...
def _experiment_fields(experiment: ExactLawArticleExperimentHandler) -> dict[str, str]:
    ...
    law_names = list(
        dict.fromkeys(
            [law_name for law_name, _ in pairs]
            + [
                law_name
                for trace in experiment.law_name_filter_traces
                for law_name in trace.law_names
            ]
        )
    )
    exact_results = [item for trace in traces for item in trace.exact_results]
    return {
        "적용된LawNames": " | ".join(law_names),
        "탐지LawArticlePairs": " | ".join(f"{law} {article}" for law, article in pairs),
        "ExactLookupAttempted": "Y" if any(trace.attempted for trace in traces) else "N",
        "ExactLookupHit": "Y" if any(trace.hit for trace in traces) else "N",
        "ExactLookupResult": " | ".join(
            f"{item.get('law_name', '')} {item.get('article_number', '')}".strip()
            for item in exact_results
        ),
        "SemanticFallback여부": "Y" if any(trace.semantic_fallback for trace in traces) else "N",
        "ExactLookupTrace": json.dumps(
